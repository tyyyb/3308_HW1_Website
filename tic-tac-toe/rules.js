...
function toggle_move() {
	...
	document.getElementById("turn_info").innerHTML = "Turn for : " +symbol[this.turn].bold();
...
function validate(){
	if(board_state[0] === board_state[1] && board_state[1] === board_state[2]){
		...
		return board_state[1] === -1?null:board_state[1]
	}else if(board_state[2] === board_state[5] &&board_state[3]=== board_state[8]){
		return board_state[2] === -1?null:board_state[2]
...
function play() {
	...
	move = 	document.getElementById("move_text_id").value;
	if(table_ids.includes(move) && board_state[table_ids.indexOf(move)] === -1){
		document.getElementById(move).innerText = symbol[whose_move()];
		board_state[table_ids.indexOf(move)] = whose_move();
		toggle_move();
		if(validate() !== null){
			setTimeout(function(){alert("Winner is : "+ symbol[validate()])},10);
			setTimeout(function(){reset_play()},20);
		}
	}else{
		alert("Invalid input")
	}
